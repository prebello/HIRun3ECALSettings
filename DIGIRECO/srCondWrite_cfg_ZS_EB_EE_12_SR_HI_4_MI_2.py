...
process.CondDBCommon.connect = 'sqlite_file:EcalSRSettings_beam2018_option1_v00_mc_ZS_EB_EE_12_SR_HI_4_MI_2.db'
...
process.PoolDBOutputService = cms.Service("PoolDBOutputService",
    ...
    toPut = cms.VPSet(cms.PSet(
        ...
        tag = cms.string('EcalSRSettings_beam2018_option1_v00_mc_ZS_EB_EE_12_SR_HI_4_MI_2')
    ))
)
...
#    mode = cms.string("online_config"), #import online SRP config from onlineSrpConfigFile file and bxGlobalOffset,
    ...
    #for debug mode only:
    trigPrimBypassLTH = cms.double(2.0),  #value changed Pat

    #for debug mode only:
    trigPrimBypassHTH = cms.double(4.0), #value changed Pat
...
# INFO: 
# - leave the AdcToGeV unchanged
# - the first number in LowInterestChannelZS is the one that matters,
#   it's in ADC, you can change it. FYI EE has been to 6 for the entire Run2, EB went
#   from 2.75 (2016) to 4.5 (2017) to 5.5 (2018)
#   more info at: https://cmsonline.cern.ch/webcenter/portal/cmsonline/Common/Elog?_piref683379043.strutsAction=/viewMessageDetails.do%3fmsgId=1063772
#   Note that the values there has to be devided by 4 so the stadard config for EB
#   is 22/4 = 5.5
# ZS energy threshold in GeV to apply to low interest channels of barrel
process.writeInDB.ebDccAdcToGeV = cms.double(0.035)
process.writeInDB.srpBarrelLowInterestChannelZS = cms.double(12.0*0.035) #value changed here Pat

#added Pat
process.writeInDB.srpBarrelHighInterestChannelZS = cms.double(12.0*0.035)

# ZS energy threshold in GeV to apply to low interest channels of endcap
process.writeInDB.eeDccAdcToGeV = cms.double(0.06)
process.writeInDB.srpEndcapLowInterestChannelZS = cms.double(12.0*0.06)  #value changed here Pat

#added Pat
process.writeInDB.srpEndcapHighInterestChannelZS = cms.double(12.0*0.035)
